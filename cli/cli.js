...
// Identifiers for killAllAppProcesses - only kill lazyrouter specifically
const PROCESS_IDENTIFIERS = [
  'lazyrouter'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "lazyrouter")
    : path.join(os.homedir(), ".lazyrouter");
}
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          lines.forEach(line => {
            ...
            // Avoids killing editors/grep/strace/cursor that just have "lazyrouter" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("lazyrouter") && (cmd.includes("cli.js") || cmd.includes("\\9router") || cmd.includes("/9router")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running 9router/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "lazyrouter".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("lazyrouter") && (cmd.includes("cli.js") || cmd.includes("/9router")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 LazyRouter is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 LazyRouter is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "lazyrouter", "db.json") : path.join(".lazyrouter", "db.json"));
