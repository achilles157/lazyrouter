...
// Upstream drives a headless Chromium through playwright-core (with a
// hardcoded chromium path from the author's own machine) to mint an Aliyun
// captcha token for ZCode's web signup flow. That helper is not portable and
// playwright-core is not a dependency of this fork, so no token is sent here.
// API keys issued through the normal flow do not need one.
let _cachedCaptcha = null;
let _captchaExpiry = 0;
let _captchaPromise = null;

async function solveCaptcha(log) {
  log?.debug?.("CAPTCHA", "browser captcha solver disabled in this fork; continuing without a captcha param");
  return null;
}
