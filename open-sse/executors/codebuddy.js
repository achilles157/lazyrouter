...
export class CodeBuddyGlobalExecutor extends DefaultExecutor {
  ...
  buildHeaders(credentials) {
    ...
    // Cookie-based auth: if no API key / access token but webCookie is available,
    // use the captured browser session cookie instead (bypasses Pro plan requirement).
    const psd = credentials.providerSpecificData || {};
    if (psd.webCookie && !credentials.apiKey && !credentials.accessToken) {
      headers["Cookie"] = psd.webCookie;
      // Remove Authorization header if set by super (would be empty/invalid anyway)
