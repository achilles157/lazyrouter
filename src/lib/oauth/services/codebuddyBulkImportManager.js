...
async function completeCodeBuddyRegistration(page, onStep) {
  ...
  try {
    ...
    // Force language to Intl English — prevents Keycloak login page from showing
    // in Chinese (which happens when browser locale or prior session was zh-CN).
    await page.evaluate(() => {
      try { localStorage.setItem("CODEBUDDY_IDE_STORAGE_LANG", "en-US"); } catch {}
    }).catch(() => null);
