...
async function handleProviderLoginGate(page, reportStep) {
  ...
  if (checkedTerms) {
    ...
    // Immediately attempt Google click in the same call after checking the
    // policy checkbox. Returning true here without clicking Google would
    // cause the next loop iteration to see an already-checked checkbox,
    // return true again, and Google would never get clicked.
    const clickedGoogleAfterTerms = await clickFirstActionable(page, GOOGLE_LOGIN_BUTTON_SELECTORS);
    if (clickedGoogleAfterTerms) {
      reportStep("selecting_google_login", "Selecting Google login after terms acceptance");
      await page.waitForTimeout(1000);
      const confirmedDialogAfterTerms = await clickFirstActionable(page, PRIVACY_CONFIRM_BUTTON_SELECTORS);
      if (confirmedDialogAfterTerms) {
        reportStep("accepting_provider_privacy_dialog", "Confirmed provider privacy agreement dialog");
        await page.waitForTimeout(1000);
      }
    }
