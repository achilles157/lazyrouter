...
export const BULK_IMPORT_PROVIDERS = Object.freeze({
  ...
  antigravity: {
    ...
    staleOnLatest404: true,
    parseAccounts: (accounts) =>
      import("@/lib/oauth/services/antigravityBulkImportManager").then((m) => m.parseAntigravityBulkAccounts(accounts)),
    getManager: () =>
      import("@/lib/oauth/services/antigravityBulkImportManager").then((m) => m.getAntigravityBulkImportManager()),
    normalizeStartArgs: (body, resolvedProxy) => ({
      accounts: body?.accounts ?? [],
      concurrency: body?.concurrency,
      engine: body?.engine,
