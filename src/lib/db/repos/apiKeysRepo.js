...
// Parse a JSON TEXT column with null=all / []=none semantics (ACL tri-state).
...
export async function updateApiKey(id, data) {
  ...
  db.transaction(() => {
    ...
    // Merge: only override fields explicitly present in data (so
    // allowedProviders: null clears, absence keeps the current value).
    const merged = { ...current };
    if (data.isActive !== undefined) merged.isActive = data.isActive;
    if (data.name !== undefined) merged.name = data.name;
    if (data.machineId !== undefined) merged.machineId = data.machineId;
