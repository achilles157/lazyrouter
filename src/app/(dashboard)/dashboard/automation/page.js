      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        <button type="button" onClick={() => setIsBulkOpen(true)} className="text-left">
          <Card
            ...
            icon="group_add"
            title="Auto Login Bulk"
            subtitle="Run bulk gmail|password automation via Google OAuth. Automatically onboards each account to Gemini Code Assist and saves the connection."
          />
        </button>
      </div>
      <BulkAccountAutomationModal
...
const AUTOMATION_PROVIDERS = [
  ...
  },
  {
    ...
    label: "Antigravity",
    icon: "rocket_launch",
    description: "Bulk GSuite auto login via Google OAuth with automatic Code Assist onboarding.",
