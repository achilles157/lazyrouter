...
export default function TokenSaverClient() {
  ...
  const [guards, setGuards] = useState({ loopGuard: true, circuitBreaker: true });
    ...
    const loadSettings = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setGuards({
            loopGuard: data.loopGuardEnabled !== false && data.loopGuardEnabled !== 0,
            circuitBreaker: data.circuitBreakerEnabled !== false && data.circuitBreakerEnabled !== 0,
          });
          refreshHeadroomStatus();
